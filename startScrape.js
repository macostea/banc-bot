...
async function main() {
  ...
  for (category of categories) {
    ...
    fs.appendFileSync("data.sql", `INSERT INTO categories (Name, Url, TotalBancs) SELECT '${category.name}', '${category.url}', '${category.totalBancs}' WHERE NOT EXISTS (SELECT 1 FROM categories WHERE Url = '${category.url}');\n`);

    const totalPages = Math.trunc((category.totalBancs - 1) / 20) + 1;
    for (let pageNumber = 1; pageNumber <= totalPages; pageNumber++) {
      const pageUrl = category.url.replace(".htm", "") + "/pagina-" + pageNumber + ".htm";
      ...
      for (const banc of bancs) {
        fs.appendFileSync("data.sql", `INSERT INTO bancs (Text, CategoryId) SELECT '${banc}', (SELECT Id FROM categories WHERE Name = '${category.name}') WHERE NOT EXISTS (SELECT 1 FROM bancs WHERE Text = '${banc}');\n`);
      }
    }
  }
